...
export default function App() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    ctx.fillStyle = navy;
    ctx.fillRect(0, 0, 1240, 292);
    ctx.fillStyle = gold;
    ctx.fillRect(0, 292, 1240, 9);
    ctx.textAlign = "center";
    ctx.fillStyle = "#fff";
    ctx.font = '700 31px "Times New Roman"';
    ctx.fillText(data.storeName || "TÊN CỬA HÀNG", 620, 78);
    ctx.fillStyle = "#d7c49d";
    ctx.font = '16px "Times New Roman"';
    ctx.fillText("UY TÍN TẠO NÊN GIÁ TRỊ", 620, 111);
    ctx.font = '700 52px "Times New Roman"';
    ctx.fillStyle = "#fff";
    ctx.fillText("PHIẾU BẢO HÀNH", 620, 190);
    ctx.font = '18px "Times New Roman"';
    ctx.fillStyle = "#dfe5ec";
    ctx.fillText(`MÃ PHIẾU: ${warrantyCode}`, 620, 234);
    const line = (y: number) => {
    ...
    const section = (title: string, y: number) => {
      ctx.textAlign = "left";
      ctx.fillStyle = gold;
      ctx.font = '700 18px "Times New Roman"';
      ctx.fillText(title.toUpperCase(), 86, y);
      line(y + 20);
    };
    const field = (label: string, value: string, x: number, y: number, width = 470) => {
      ...
      ctx.fillStyle = ink;
      ctx.font = '700 23px "Times New Roman"';
      wrap(value, x, y + 37, width, 30);
    };
    section("Thông tin cửa hàng", 363);
    field("Tên cửa hàng", data.storeName, 86, 423);
    field("Số điện thoại", data.storePhone, 670, 423);
    field("Địa chỉ cửa hàng", data.storeAddress, 86, 520, 470);
    field("Địa chỉ kho", data.warehouseAddress, 670, 520, 470);
    section("Thông tin khách hàng", 632);
    field("Tên khách hàng", data.customerName, 86, 692);
    field("Số điện thoại", data.customerPhone, 670, 692);
    field("Địa chỉ", data.customerAddress, 86, 789, 1068);
    section("Thông tin sản phẩm", 875);
    ctx.fillStyle = muted;
    ctx.font = '700 14px "Times New Roman"';
    ctx.fillText("STT", 86, 918);
    ctx.fillText("LOẠI ĐỒNG HỒ", 165, 918);
    ctx.fillText("MÀU", 770, 918);
    const visibleProducts = data.products.slice(0, 4);
    visibleProducts.forEach((product, index) => {
      const y = 950 + index * 27;
      ctx.fillStyle = ink;
      ctx.font = '16px "Times New Roman"';
      ctx.fillText(String(index + 1), 96, y);
      ctx.fillText(product.watchType || "—", 165, y);
      ctx.fillText(product.color || "—", 770, y);
    });
    if (data.products.length > 4) {
      ctx.fillStyle = muted;
      ctx.font = 'italic 14px "Times New Roman"';
      ctx.fillText(`+ ${data.products.length - 4} sản phẩm khác`, 165, 1058);
    }
    section("Thời hạn bảo hành", 1075);
    field("Ngày mua", formatDate(data.purchaseDate), 86, 1118);
    field("Ngày hết hạn", formatDate(data.expiryDate), 670, 1118);
    ...
    ctx.fillRect(86, 1510, 1068, 1);
    ctx.textAlign = "center";
    ctx.fillStyle = ink;
    ctx.font = '700 17px "Times New Roman"';
    ctx.fillText("KHÁCH HÀNG", 300, 1552);
    ctx.fillText("ĐẠI DIỆN CỬA HÀNG", 940, 1552);
    ctx.fillStyle = muted;
    ctx.font = 'italic 15px "Times New Roman"';
    ctx.fillText("(Ký và ghi rõ họ tên)", 300, 1582);
    ctx.fillText("(Ký và ghi rõ họ tên)", 940, 1582);
    ctx.fillStyle = navy;
    ctx.fillRect(0, 1658, 1240, 96);
    ctx.fillStyle = "#fff";
    ctx.font = '16px "Times New Roman"';
    ctx.fillText(
